...
-- 2026-04-09
-- 目标：
-- 1. 扩展 security_scan_runs，为后续扫描重构补充运行快照字段
-- 2. 新增统一目标表、证据表、运行命中表
-- 3. 保持幂等，兼容开发环境重复执行
...
SET @sql = IF(
  EXISTS(
    ...
    WHERE TABLE_SCHEMA = @schema_name
      AND TABLE_NAME = 'security_scan_runs'
      AND COLUMN_NAME = 'run_no'
  ),
  'SELECT 1',
  "ALTER TABLE `security_scan_runs` ADD COLUMN `run_no` INT NULL COMMENT '同一任务下的运行序号' AFTER `task_id`"
);
...
SET @sql = IF(
  ...
  'SELECT 1',
  "ALTER TABLE `security_scan_runs` ADD COLUMN `phase` VARCHAR(30) NULL COMMENT '当前阶段: prepare/discovery/verification/authenticated/reporting/completed' AFTER `status`"
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(
  ...
  'SELECT 1',
  "ALTER TABLE `security_scan_runs` ADD COLUMN `config_snapshot` JSON NULL COMMENT '本次执行配置快照' AFTER `message`"
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(
  ...
  'SELECT 1',
  "ALTER TABLE `security_scan_runs` ADD COLUMN `target_snapshot` JSON NULL COMMENT '本次展开目标快照' AFTER `config_snapshot`"
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(
  EXISTS(
    ...
    WHERE TABLE_SCHEMA = @schema_name
      AND TABLE_NAME = 'security_scan_runs'
      AND COLUMN_NAME = 'summary_snapshot'
  ),
  'SELECT 1',
  "ALTER TABLE `security_scan_runs` ADD COLUMN `summary_snapshot` JSON NULL COMMENT '本次统计摘要快照' AFTER `target_snapshot`"
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(
  ...
  'SELECT 1',
  "ALTER TABLE `security_scan_runs` ADD COLUMN `cancelled_at` DATETIME NULL COMMENT '取消时间' AFTER `completed_at`"
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(
  EXISTS(
    ...
    WHERE TABLE_SCHEMA = @schema_name
      AND TABLE_NAME = 'security_scan_runs'
      AND INDEX_NAME = 'idx_security_scan_runs_run_no'
  ),
  'SELECT 1',
  "ALTER TABLE `security_scan_runs` ADD INDEX `idx_security_scan_runs_run_no` (`task_id`, `run_no`)"
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(
  EXISTS(
    ...
    WHERE TABLE_SCHEMA = @schema_name
      AND TABLE_NAME = 'security_scan_runs'
      AND INDEX_NAME = 'idx_security_scan_runs_phase'
  ),
  'SELECT 1',
  "ALTER TABLE `security_scan_runs` ADD INDEX `idx_security_scan_runs_phase` (`phase`)"
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 为已有运行记录补 run_no，按 task_id + id 递增
UPDATE `security_scan_runs` AS target_run
JOIN (
  SELECT ranked.id, ranked.run_no
  FROM (
    SELECT
      source_run.id,
      source_run.task_id,
      @seq := IF(@prev_task = source_run.task_id, @seq + 1, 1) AS run_no,
      @prev_task := source_run.task_id AS current_task_marker
    FROM `security_scan_runs` AS source_run
    JOIN (SELECT @prev_task := 0, @seq := 0) AS vars
    ORDER BY source_run.task_id, source_run.id
  ) AS ranked
) AS backfill
  ON backfill.id = target_run.id
SET target_run.run_no = backfill.run_no
WHERE target_run.run_no IS NULL;
...
SET `phase` = CASE
  ...
  WHEN `status` = 'running' THEN 'legacy-running'
  WHEN `status` = 'pending' THEN 'legacy-pending'
  ELSE 'legacy'
END
WHERE `phase` IS NULL;
