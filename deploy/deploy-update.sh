...
validate_remote_env() {
    ...
    case "${DEPLOY_TARGET}" in
        ...
        backend)
            required_keys=(DB_PASSWORD REDIS_PASSWORD JWT_SECRET)
            optional_warn_keys=(GRAFANA_URL GRAFANA_USERNAME GRAFANA_PASSWORD JENKINS_URL JENKINS_USERNAME JENKINS_TOKEN)
            ;;
        nginx)
        ...
        all)
            ...
            optional_warn_keys=(GRAFANA_URL GRAFANA_USERNAME GRAFANA_PASSWORD GRAFANA_BASIC_AUTH JENKINS_URL JENKINS_USERNAME JENKINS_TOKEN)
            ;;
    esac
