...
export default function AlertRulesPage() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (mapped.length === 0) {
        message.warning('未从 Prometheus 获取到告警规则')
        return
      }
      ...
      message.error('同步 Prometheus 规则失败')
    } finally {
      setSyncing(false)
    }
  }
  ...
  const columns = [
    ...
    { title: '规则名称', dataIndex: 'name', key: 'name', width: 250, ellipsis: true },
    {
    ...
    },
    {
      ...
      render: (v: string) => <Tag color={categoryConfig[v]?.color}>{categoryConfig[v]?.text || v}</Tag>,
    },
    { title: '规则组', dataIndex: 'rule_group', key: 'rule_group', width: 150, ellipsis: true },
    {
      title: '告警状态', dataIndex: 'grafana_state', key: 'grafana_state', width: 120,
      render: (v: string) => {
        if (!v) return '-'
    ...
    {
      title: '报警组', dataIndex: 'group', key: 'group', width: 120,
      render: (g: AlertNotifyGroup | undefined) => g ? <Tag color="blue">{g.name}</Tag> : <span style={{ color: '#999' }}>未配置</span>,
    },
    {
      title: '最后同步', dataIndex: 'synced_at', key: 'synced_at', width: 170,
      render: (v: string) => {
        if (!v) return '-'
        const d = new Date(v)
    ...
    },
    {
      ...
      render: (_: unknown, r: AlertRule) => (
        <Space size={4}>
          {canEdit() && <Button type="link" size="small" icon={<EditOutlined />} onClick={() => handleEditRule(r)}>配置</Button>}
          {canEdit() && <Popconfirm title="确定删除此规则？" onConfirm={() => handleDeleteRule(r.id)}>
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between' }}>
        ...
        <span style={{ color: '#999', fontSize: 13 }}>共 {rules.length} 条规则</span>
      </div>
          ...
          expandedRowRender: (record: AlertRule) => (
            <div style={{ padding: '8px 0' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr', gap: '8px 12px', fontSize: 13 }}>
                <span style={{ color: '#999', fontWeight: 500 }}>触发条件:</span>
                <code style={{
                  background: '#f6f8fa', padding: '4px 8px', borderRadius: 4,
                  fontSize: 12, color: '#d63384', wordBreak: 'break-all',
                }}>
                  {record.expression || '未配置'}
                </code>
                {record.condition && (
                  <>
                    <span style={{ color: '#999', fontWeight: 500 }}>条件说明:</span>
                    <span>{record.condition}</span>
                  </>
                )}
                {record.description && (
                  <>
                    <span style={{ color: '#999', fontWeight: 500 }}>规则描述:</span>
                    <span>{record.description}</span>
                  </>
                )}
                {record.folder_title && (
                  <>
                    <span style={{ color: '#999', fontWeight: 500 }}>规则来源:</span>
                    <span style={{ color: '#666' }}>{record.folder_title}</span>
                  </>
                )}
                {record.grafana_uid && (
                  <>
                    <span style={{ color: '#999', fontWeight: 500 }}>规则标识:</span>
      ...
      <Modal title="配置告警规则" open={editModal.open} onOk={handleSaveRule} onCancel={() => setEditModal({ open: false })} width={550}>
        <div style={{ marginBottom: 16, padding: 12, background: '#f5f5f5', borderRadius: 6 }}>
          <b>{editModal.rule?.name}</b>
          {editModal.rule?.expression && <div style={{ fontSize: 12, color: '#888', marginTop: 4 }}>表达式: {editModal.rule.expression}</div>}
        </div>
        <Form form={form} layout="vertical">
          <Form.Item name="severity" label="告警级别" rules={[{ required: true }]}>
            <Select options={[{ label: '严重', value: 'critical' }, { label: '警告', value: 'warning' }, { label: '提醒', value: 'info' }]} />
          </Form.Item>
          <Form.Item name="category" label="告警分类" rules={[{ required: true }]}>
            <Select options={Object.entries(categoryConfig).map(([k, v]) => ({ label: v.text, value: k }))} />
          </Form.Item>
          <Form.Item name="alert_group_id" label="关联报警组">
            <Select allowClear placeholder="选择报警组" options={groups.map(g => ({ label: g.name, value: g.id }))} />
          </Form.Item>
          <Form.Item name="condition" label="触发条件说明">
            <Input.TextArea rows={2} placeholder="如：CPU 使用率持续 5 分钟超过 90%" />
          </Form.Item>
