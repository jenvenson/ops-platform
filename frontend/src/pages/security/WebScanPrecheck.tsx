...
const levelMeta: Record<PrecheckLevel, { color: string; text: string }> = {
  direct: { color: 'green', text: '可直接接入' },
  custom: { color: 'gold', text: '需要定制认证流' },
  blocked: { color: 'red', text: '暂不建议自动扫描' },
}
...
function inferResult(values: WebScanPrecheckValues): PrecheckResult {
  ...
  if (!target) {
    level = 'custom'
    reasons.push('还没有填写目标 URL，无法判断是否适合直接接入。')
  }

  if (access === 'public') {
    level = 'custom'
    reasons.push('该站点看起来允许匿名访问，当前 Web 漏扫链路更偏向登录后扫描。')
  }

  if (authComplexity === 'custom') {
    level = 'custom'
    reasons.push('登录链路包含签名、加密密码、租户标识或动态参数，通常需要定制 auth_flow。')
  }

  if (authComplexity === 'captcha') {
    level = 'blocked'
    reasons.push('登录链路依赖验证码、短信、扫码或人机校验，当前不适合直接自动扫描。')
  }

  if (sessionCarrier === 'mixed') {
    level = level === 'blocked' ? 'blocked' : 'custom'
    reasons.push('登录后会话依赖 Cookie 和 Header 混合态，建议先人工确认会话复用方式。')
  }

  if (sessionCarrier === 'unknown') {
    reasons.push('会话载体还不明确，建议先确认登录后是 Cookie 还是 Header Token。')
  }

  if (!values.username?.trim() && access !== 'public') {
    level = level === 'blocked' ? 'blocked' : 'custom'
    reasons.push('未提供测试账号，后续即使建任务也很难完成登录态验证。')
  }

  if (containsKeyword(target, ['/open/', '/public/', '/anonymous/']) || containsKeyword(loginURL, ['oauth2', 'token'])) {
    if (authComplexity === 'unknown') {
      level = level === 'blocked' ? 'blocked' : 'custom'
      reasons.push('目标 URL 或登录接口看起来存在开放接口或 token 登录特征，建议先做接入预检查。')
    }
  }

  if (reasons.length === 0) {
    reasons.push('当前信息更接近常规账号密码 + 可复用会话的网站，可以先按标准流程接入。')
  }
  ...
  const description = level === 'direct'
    ? '建议先按标准登录后 Web 扫描创建任务；如果自动生成的 auth_flow 不稳定，再转定制认证。'
    : level === 'custom'
      ? '建议先让运维小助手协助判断登录链路，再决定是否生成或调整 auth_flow。'
      : '建议先人工复盘登录流程，确认是否存在验证码、短信、扫码或设备校验，再决定是否走自动化扫描。'

  const assistantQuery = [
    '请帮我判断这个网站能否直接接入登录后 Web 漏洞扫描。',
    `目标 URL: ${target || '-'}`,
    `登录 URL: ${loginURL || '-'}`,
    `访问要求: ${access}`,
    `登录复杂度: ${authComplexity}`,
    `会话方式: ${sessionCarrier}`,
    `测试账号: ${values.username?.trim() ? '已提供' : '未提供'}`,
    `页面预判: ${levelMeta[level].text}`,
    '如果不能直接接入，请只回答：需要补什么关键信息，或为什么暂时不建议自动扫描。',
  ].join('\n')
...
export default function WebScanPrecheckCard({ form, compact = false }: WebScanPrecheckCardProps) {
  ...
  const result = inferResult(values || {})
      ...
      <Space direction="vertical" style={{ width: '100%' }} size={12}>
        ...
        <Row gutter={16}>
          <Col span={24}>
            <Form.Item
              ...
              label="该站点是否本来就要求登录后访问"
              initialValue="login-required"
            >
              <Radio.Group optionType="button" buttonStyle="solid">
                <Radio.Button value="login-required">要求登录</Radio.Button>
                <Radio.Button value="public">允许匿名</Radio.Button>
                <Radio.Button value="unknown">暂不确定</Radio.Button>
              </Radio.Group>
            </Form.Item>
          </Col>
          <Col span={24}>
            <Form.Item
              ...
              extra="如果登录要做签名、加密密码、租户参数或动态 token，选“定制认证”。"
            >
              <Radio.Group optionType="button" buttonStyle="solid">
                <Radio.Button value="standard">常规表单/Token</Radio.Button>
                <Radio.Button value="custom">定制认证</Radio.Button>
                <Radio.Button value="captcha">验证码/短信/扫码</Radio.Button>
                <Radio.Button value="unknown">暂不确定</Radio.Button>
              </Radio.Group>
            </Form.Item>
          </Col>
        ...
        <div>
          <Paragraph type="secondary" style={{ marginBottom: 8 }}>
            如果你对登录方式、会话复用或是否需要定制认证拿不准，再让运维小助手补一层判断。
          </Paragraph>
